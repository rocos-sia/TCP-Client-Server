...
#define DEFAULT_ID 0x00 //默认灵巧手ID
...
public:
    InspireHand(int8_t id = 0x01, const std::string &serialPortName = "COM3") : _id(id),
...
public:
    ...
    /// 获取手指shou, 0-1000，-1不改动
    /// \param fingers 返回手指角度的std::vector<uint16_t>引用
    /// \return 成功标志位
    bool getForce(std::vector<uint16_t> &fingers)
    {
        //        fingers.clear();
        boost::progress_timer t;
        ProtoGetForce p(_id);
    ...
    bool setAngle(int16_t f1)
    {
        ...
        //等同 std::cout << "angles[" <(int)n1)<< "] = " << f1 << std::endl;
...
private:
    ...
    bool receive(std::vector<uint8_t> &buf, size_t n)
    { // 接收指定字节的数据

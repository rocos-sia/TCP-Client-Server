...
public:
    ...
    constexpr static inline const uint16_t ADDR_USER_DEF_ANGLE(uint8_t K, uint8_t m)
    { // 自定义手势角度值, RW, 32*6 short
...
{
public:
    explicit ProtocolBase(uint8_t id, uint8_t order, uint16_t register_addr) : _id(id),
                                                                               _order(order),
...
protected:
    template <typename D>
    inline uint8_t getBinary(D t, size_t i)
    { //获取内存中存储数值
        return ((uint8_t *)&t)[i];
    }
...
typedef ProtocolWrite<uint16_t, 6, Register::ADDR_FORCE_SET(0)> ProtoSetForce; // 设置手指力量
typedef ProtocolRead<uint16_t, 6, Register::ADDR_FORCE_ACT(0)> ProtoGetForce;  // 获取手指力量
